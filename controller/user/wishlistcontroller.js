...
const loadWishList = async (req, res) => {
    try {
        const userId = req.session.userId || (req.session.user && req.session.user._id ? req.session.user._id : req.session.user);
        if (!userId) {
        ...
        if (!wishlistDoc) {
            return res.render('wishlist', { user: user, wishlist: [] });
        }
        const productIds = wishlistDoc.products.filter(item => item.productId).map(item => item.productId);

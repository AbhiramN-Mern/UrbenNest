...
const loadhomepage = async (req, res) => {
    try {
        ...
        const userId = user ? ((typeof user === 'object' && user._id) ? user._id : user) : null;
...
const signup = async (req, res) => {
    try {
        ...
        const { email, phone, name, password, confirmPassword, referralCode } = req.body;
        ...
        if (!emailSent) {
            console.log("Email delivery failed or blocked. Generated OTP logged to console:", otp);
        }
        ...
        req.session.userData = { name, phone, email: email.toLowerCase().trim(), password, referralCode };
...
const resendOTP = async (req, res) => {
    try {
        ...
        if (!emailSent) {
            console.log("Email delivery failed or blocked. Resent OTP logged to console:", newOtp);
        }
...
const loadlogin = async (req, res) => {
  try {
    if (!req.session.user) {
      ...
      return res.render('login', { message: errorMsg, error: errorMsg });
    } else {
      res.redirect('/');
    }
  } catch (error) {
...
const login = async (req, res) => {
    try {
        ...
        if (!email || !password) {
            return res.render('login', { message: "Email and password are required", error: "Email and password are required" });
        }

        const findUser = await User.findOne({ isAdmin: { $ne: true }, email: email.toLowerCase().trim() });
        if (!findUser) {
            return res.render('login', { message: "User Not Found", error: "User Not Found" });
        }

        if (findUser.isBlocked) {
            console.log('User is Blocked by Admin');
            return res.render('login', { message: 'User is Blocked by Admin', error: 'User is Blocked by Admin' });
        }

        if (!findUser.password) {
            return res.render('login', { 
                message: "This account was registered without a password. Please use Forgot Password to set one.", 
                error: "This account was registered without a password. Please use Forgot Password to set one." 
            });
        }
        ...
        if (!passwordMatch) {
            ...
            return res.render('login', { message: "Incorrect Password", error: "Incorrect Password" });
        }
        ...
        res.render('login', { message: 'Login failed, please try again later', error: 'Login failed, please try again later' });
    }
};

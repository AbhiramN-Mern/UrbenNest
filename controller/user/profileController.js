...
const forgotEmailValid = async (req, res) => {
    try {
        console.log("forgotEmailValid:", req.body);
        ...
        if(findUser){
            ...
            try {
                ...
                console.warn("sendVerificationEmail error (falling back to console OTP):", mailErr.message);
            }
...
const resendOtp = async (req,res) =>{
    try {
        ...
        return res.status(200).json({success:true, message:"Resent OTP Successful"});
    } catch (error) {
...
const changePasswordValid = async (req,res) =>{
    try {
        ...
        if(userExists){
            const otp = generateOtp();
            console.log("Change Password OTP for", email, ":", otp);
            req.session.userOtp = otp;
...
const changeEmailValid=async(req,res)=>{
    try {
        ...
        if(userExists){
            const otp=generateOtp()
            console.log("Change Email OTP for", email, ":", otp);
            req.session.userOtp=otp;
            req.session.userData=req.body;
            req.session.email=email;
            try {
...
const updateEmail = async(req,res)=>{
    try {
        ...
        const userId = req.session.user;
        if (!newEmail) {
            return res.render('new-email', { message: 'Email cannot be empty', userData: req.session.userData });
        }
        const existingUser = await User.findOne({ email: newEmail, _id: { $ne: userId } });
        if (existingUser) {
            return res.render('new-email', { message: 'Email is already in use by another account', userData: req.session.userData });
        }
...
const postAddAddress = async (req, res) => {
    try {
        ...
        if (req.query.redirect === 'checkout' || req.body.redirect === 'checkout') {
